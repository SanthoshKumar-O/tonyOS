...
from tony.tools.process import ProcessChildrenTool


def test_metadata() -> None:
    tool = ProcessChildrenTool()

    assert tool.metadata.name == "process_children"


def test_missing_pid() -> None:
    result = ProcessChildrenTool().execute([])
...
@patch("tony.tools.process.children.Path.read_text")
def test_returns_children(mock_read_text) -> None:
    mock_read_text.return_value = "124 125 130\n"
    ...
    assert result.output == "124 125 130"
    assert result.exit_code == 0


@patch("tony.tools.process.children.Path.read_text")
def test_no_children(mock_read_text) -> None:
...
def test_permission_denied(mock_read_text) -> None:
    ...
    assert result.success is False
    assert result.error == "Permission denied reading children of process '123'."
