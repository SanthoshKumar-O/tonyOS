"""Tests for ProcessExecutableTool."""
...
from tony.tools.process import ProcessExecutableTool


def test_metadata() -> None:
    tool = ProcessExecutableTool()

    assert tool.metadata.name == "process_executable"
...
@patch("tony.tools.process.executable.Path.resolve")
def test_returns_executable(mock_resolve) -> None:
    mock_resolve.return_value = Path("/usr/bin/python3")
    ...
    assert result.output == "/usr/bin/python3"
...
def test_permission_denied(mock_resolve) -> None:
    ...
    assert (
        result.error
        == "Permission denied reading executable of process '123'."
    )
